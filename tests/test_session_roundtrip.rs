...
use mneme_substrate::activations::arbor::{ArborConfig, ArborStorage};
use mneme_substrate::activations::claudecode::sessions;
use mneme_substrate::activations::claudecode::{
...
async fn test_view_collapse_export() {
    ...
    for (i, (_node_id, node)) in view_tree.nodes.iter().enumerate().take(10) {
        use mneme_substrate::activations::arbor::NodeType;
